# -*- coding: utf-8 -*-
"""
train_nn.py - Experiment 1: Random Split
End-to-end MambaCross neural network trainer for Experiment 1 (Random Split).
"""
import os
import sys
import json
...
def load_split():
    ...
    # Fallback to master dataset split column
...
def main(epochs=30, batch_size=32):
    ...
    print(f"Training Exp 1 (Random Split) on {device} for {epochs} epochs...")
    ...
    print(f"✓ [Exp 1] Best AUC={best_auc:.4f} (epoch {best_metrics.get('Best Epoch')}). Saved -> {results_json}")
    return best_metrics


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ESM-Mamba on Experiment 1 (Random Split)")

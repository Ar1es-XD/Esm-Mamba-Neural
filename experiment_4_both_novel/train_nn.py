# -*- coding: utf-8 -*-
"""
train_nn.py - Experiment 4: Both Novel (Double Holdout)
End-to-end MambaCross neural network trainer for Experiment 4 (Both Novel).
"""
import os
import sys
import json
...
def load_split():
    """Load train/test pairs for Both Novel (both_block) partitioning (prefer local folder CSVs)."""
...
def main(epochs=30, batch_size=32):
    ...
    print(f"Training Exp 4 (Both Novel) on {device} for {epochs} epochs...")
    ...
    print(f"✓ [Exp 4] Best AUC={best_auc:.4f} (epoch {best_metrics.get('Best Epoch')}). Saved -> {results_json}")
    return best_metrics


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ESM-Mamba on Experiment 4 (Both Novel)")
